...
[plan.md](./plan.md) に従い、Phase 4 の Diff graph の統合と選択を実装する。ユーザーが明示的に変更しない限り、同ファイルを仕様の正として扱うこと。
...
- CI、graph の差分統合と選択、Mermaid、配布ドキュメントは未実装。Git 基盤と TypeScript 解析の CLI 統合は Phase 6 で行う。
- npm publish は行っていない。
...
2. Phase 4 から順に進め、各 phase で関連 test を同時に追加する。
